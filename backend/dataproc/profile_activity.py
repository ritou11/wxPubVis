...
for i, pro in enumerate(procur):
    ...
    pstcur = pstcol.find({'msgBiz': pro['msgBiz']},
                         projection={'readNum': 1,
                                     'likeNum': 1, 'publishAt': 1},
    ...
    for j, pst in enumerate(pstcur):
        if (j % 10 == 0):
            print(
                f'\rProfile:{i + 1}/{totalPro} Post:{j + 1}/{totalPst}', end='', flush=True)
        if 'readNum' not in pst.keys() or 'publishAt' not in pst.keys():
            continue
        likeNum = pst['likeNum'] if 'likeNum' in pst.keys() and pst['likeNum'] else 0
        readNum = pst['readNum'] if 'readNum' in pst.keys() and pst['readNum'] else 0
        if pst['publishAt'] not in data.keys():
            data[pst['publishAt']] = [
                {'readNum': readNum, 'likeNum': likeNum}]
        else:
            data[pst['publishAt']].append(
                {'readNum': readNum, 'likeNum': likeNum})
    pstcur.close()
    ...
    for k in data.keys():
        ...
        res = reduce(lambda x, y: {
                     'readNum': x['readNum'] + y['readNum'],
                     'likeNum': x['likeNum'] + y['likeNum']
                    }, data[k])
